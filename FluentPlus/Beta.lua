...
Components.Tab = (function()
	...
	function TabModule:New(Title, Icon, Parent)
		...
		function Tab:AddRow(ColumnCount)
			ColumnCount = math.clamp(ColumnCount or 2, 1, 4)
			local targetContainer = Tab.Container
			if self.SelectedSubTab > 0 and self.SubTabs[self.SelectedSubTab] then
				targetContainer = self.SubTabs[self.SelectedSubTab].Container
			end
			...
			for i = 1, ColumnCount do
				...
				local ColFrame = New("Frame", {
					Size = UDim2.new(1 / ColumnCount, -math.ceil(6 * (ColumnCount - 1) / ColumnCount), 0, 0),
					AutomaticSize = Enum.AutomaticSize.Y,
					BackgroundTransparency = 1,
					LayoutOrder = i,
					Parent = RowFrame,
				...
				local Column = {
					Type = "Column",
					Container = ColFrame,
					ScrollFrame = targetContainer,
				}

				function Column:AddSection(SectionTitle, SectionIcon)
					local Section = { Type = "Section" }
					local Icon = SectionIcon
					if not fischbypass then
						if Library:GetIcon(Icon) then
					...
					Section.Container = SectionFrame.Container
					Section.ScrollFrame = targetContainer
					setmetatable(Section, Elements)
					return Section
				end

				setmetatable(Column, Elements)
				Columns[i] = Column
			end
...
Elements.AddButtonRow = function(self, Buttons)
	assert(type(Buttons) == "table" and #Buttons > 0, "AddButtonRow - Must provide a list of buttons")
	...
	for i, BtnConfig in ipairs(Buttons) do
		...
		local Btn = New("TextButton", {
			Size = UDim2.new(1 / #Buttons, -math.ceil(5 * (#Buttons - 1) / #Buttons), 1, 0),
			...
			New("TextLabel", {
				Text = BtnConfig.Title or ("Button " .. i),
				Size = UDim2.fromScale(1, 1),
				BackgroundTransparency = 1,
		...
		Creator.AddSignal(Btn.MouseButton1Click, function()
			Library:SafeCallback(btnCallback)
		end)
	...
	return { Frame = RowFrame, Buttons = btnObjects }
end
...
Library.CreateStatusBar = function(self, Config)
	Config = Config or {}
	local ShowFPS = Config.FPS ~= false
	local ShowPing = Config.Ping ~= false
	local CustomFields = Config.Fields or {}
	local Position = Config.Position or UDim2.new(1, -10, 0, 10)
	local Anchor = Config.AnchorPoint or Vector2.new(1, 0)

	local StatusBarLayout = New("UIListLayout", {
		FillDirection = Enum.FillDirection.Horizontal,
	...
	if ShowFPS then
		order += 1
		fpsLabel = makeLabel("FPS", order)
	end
	if ShowPing then
		order += 1
		pingLabel = makeLabel("Ping", order)
	end
	for _, field in ipairs(CustomFields) do
		order += 1
		makeLabel(field.Name or ("Field" .. order), order)
	end
	...
	function StatusBar:SetField(name, value)
		if labels[name] then
			labels[name].Text = tostring(name) .. ": " .. tostring(value)
		end
	end

	function StatusBar:SetVisible(v)
	...
	Library.StatusBar = StatusBar
	return StatusBar
end

Library.CreateKeySystem = function(self, Config)
	assert(Config, "KeySystem - Missing Config")
	assert(Config.Key or Config.Keys, "KeySystem - Must provide Key or Keys table")

	local title = Config.Title or "Key System"
	local subtitle = Config.Subtitle or "Enter your key to continue"
	local validKeys = Config.Keys or { Config.Key }
	local savedKeyName = Config.SaveKey and (Config.FolderName or "FluentKeySystem")
	local noteText = Config.Note or nil
	local url = Config.URL or nil
	local urlText = Config.URLText or "Get Key"

	if savedKeyName then
		pcall(function()
			if not isfolder(savedKeyName) then makefolder(savedKeyName) end
		end)
		local keyPath = savedKeyName .. "/key.txt"
		pcall(function()
		...
		pcall(function()
			if isfile(keyPath) then
				local saved = readfile(keyPath)
				for _, k in ipairs(validKeys) do
					if saved == k then savedValid = true end
				end
			end
		end)
		if savedValid then
	...
	local KeyBox = New("Frame", {
		...
		New("UIStroke", { Transparency = 0.5, Thickness = 1.5, ThemeTag = { Color = "AcrylicBorder" } }),
		New("UIPadding", {
	...
	local InputBox = New("TextBox", {
		...
		ThemeTag = {
			TextColor3 = "Text",
			BackgroundColor3 = "Element",
			PlaceholderColor3 = "DropdownOption",
		},
	}, {
		New("UICorner", { CornerRadius = UDim.new(0, 6) }),
		New("UIPadding", {
	...
	local btnWidth = url and UDim2.new(0.5, -4, 1, 0) or UDim2.new(1, 0, 1, 0)
	...
	Creator.AddSignal(VerifyBtn.MouseButton1Click, function()
		...
		if valid then
			if savedKeyName then
				pcall(function()
					writefile(savedKeyName .. "/key.txt", entered)
				end)
			end
			keyResult = true
			resolved = true
