...
local function CreateStroke(parent, color, thickness, trans)
	...
	s.Color = color or (theme() and theme().Border) or Color3.fromRGB(255, 255, 255)
...
-- Accent-tracking shortcut: register + set immediately.
-- Use for static (non-tweened) accent surfaces so SetAccent can update them live.
local function tagAccent(inst, prop)
	pcall(function()
		inst[prop] = theme().Accent
		registerThemed(inst, prop, "Accent")
	end)
end
...
-- Mutate accent of the current theme live. All `tagAccent`-registered
-- surfaces (tab indicator, scrollbars, slider fill, buttons, minimizer,
-- progFill, etc.) update on the next applyThemeAll tick.
function SkrilyaLib:SetAccent(color)
...
function SkrilyaLib:Window(config)
	...
	-- Open-in animation: spring-scale the window up from 0.92 via UIScale.
	-- Children of mainFrame get pulled along for free. Wrapped in pcall
	-- because some ancient executors nullify Instance.new("UIScale").
	pcall(function()
	...
	tagAccent(accentDot, "BackgroundColor3")
	...
	sidebar.Parent = body
	tagAccent(sidebar, "ScrollBarImageColor3")
	...
	function Window:GlobalSetting(cfg)
		...
		if cfg.AcrylicBlur ~= nil then
			pcall(function()
				...
				local blur = lighting:FindFirstChild("SkrilyaBlur")
				local dof = lighting:FindFirstChild("SkrilyaDOF")
				...
				if cfg.AcrylicBlur then
					...
					-- Depth-of-field pass to flatten everything regardless of
					-- camera distance. FarIntensity + NearIntensity at max
					-- with a tiny in-focus window means nothing in the scene
					-- is "sharp", mimicking a true acrylic layer.
					if not dof then
					...
					-- Sheen gradient overlay on the window — adds the
					-- frosted "light edge" that pure blur can't do. One
					-- gradient, recreated each toggle to stay idempotent.
					pcall(function()
						local existing = mainFrame:FindFirstChild("_AcrylicSheen")
						if existing then existing:Destroy() end
						local sheen = Instance.new("Frame")
						sheen.Name = "_AcrylicSheen"
						sheen.Size = UDim2.new(1, 0, 1, 0)
						sheen.BackgroundColor3 = Color3.new(1, 1, 1)
						sheen.BackgroundTransparency = 0.88
						sheen.BorderSizePixel = 0
						sheen.ZIndex = 0
						CreateCorner(sheen, UI.RadiusLarge)
						local g = Instance.new("UIGradient")
					...
					pcall(function()
						local s = mainFrame:FindFirstChild("_AcrylicSheen")
	...
	function Window:_createTab(cfg, orderBase)
		...
		tagAccent(indicator, "BackgroundColor3")
		...
		tagAccent(contentFrame, "ScrollBarImageColor3")
		CreatePadding(contentFrame, UI.Pad, UI.Pad, UI.PadLarge, UI.PadLarge)
		...
		-- Expose for Window:SelectTab (avoid MouseButton1Click:Fire hack)
		Tab._select = selectThis
		...
		function Tab:CreateSection(cfg)
			...
			function Section:Slider(cfg)
				...
				-- Unified begin handler: mouse OR touch. Start from input.Position
				-- instead of Mouse.X — reliable on touch devices and executors
				-- that don't hook Mouse cleanly.
			...
			function Section:Dropdown(cfg)
				...
				-- Chip row — for multi-select, shown right-aligned in the header,
				-- overlapping selLbl when any options are picked. Horizontal flex,
				-- filled right-to-left so the last-added chip stays visible when
				-- overflow clips from the left.
				local chipRow, chipLayout
				if multi then
					chipRow = Instance.new("Frame")
					chipRow.Name = "ChipRow"
					chipRow.AnchorPoint = Vector2.new(1, 0.5)
					chipRow.Position = UDim2.new(1, -14, 0, (hasDesc and 6 or 0) + ((hasDesc and 20 or headerH) / 2))
					chipRow.Size = UDim2.new(0.6, 0, 0, 18)
					chipRow.BackgroundTransparency = 1
					chipRow.ClipsDescendants = true
					chipRow.Visible = false
					chipRow.ZIndex = 4
					chipRow.Parent = frame

					chipLayout = Instance.new("UIListLayout")
				...
				-- Build/refresh the chip row — called on init and on every toggle.
				-- Chips are read-only (click-through): the header button above
				-- absorbs clicks to open the dropdown, and option toggling
				-- happens inside the popup. We still lay chips out here so the
				-- collapsed header shows the current selection visually.
				local function rebuildChips()
					if not chipRow then return end
					for _, child in ipairs(chipRow:GetChildren()) do
						if child:IsA("GuiObject") then child:Destroy() end
					end
					if not multi or type(obj.Value) ~= "table" then
						chipRow.Visible = false
						return
					end
					local names = {}
					...
					-- Reserve the last slot for a count badge when > 2 items.
					-- With 1–2 items we render them all; with 3+ we render the
					-- first 1 and a "+N" chip for the rest.
					local show, hidden
					if #names <= 2 then
						show, hidden = names, 0
					else
						show, hidden = {names[1]}, #names - 1
					end

					for i, name in ipairs(show) do
						...
						chip.ZIndex = 5
						CreateCorner(chip, 4)
						CreatePadding(chip, 0, 0, 6, 6)
						chip.Parent = chipRow
						-- Note: not registerThemed-tracked — chips are destroyed/
						-- rebuilt on every selection change, so stale entries
						-- would leak into the `_themed` list. Accent changes
						-- propagate on the next user interaction.

						local lblC = Instance.new("TextLabel")
						lblC.Size = UDim2.new(0, 0, 1, 0)
						lblC.AutomaticSize = Enum.AutomaticSize.X
						lblC.BackgroundTransparency = 1
						lblC.Text = name
						lblC.TextColor3 = t.TextPrimary
						lblC.FontFace = t.FontBold
						lblC.TextSize = 10
						lblC.ZIndex = 6
						lblC.Parent = chip
					end
					...
					if hidden > 0 then
						...
						more.Size = UDim2.fromOffset(0, 16)
						more.AutomaticSize = Enum.AutomaticSize.X
						more.BackgroundColor3 = t.Secondary
						more.BackgroundTransparency = 0.3
						more.BorderSizePixel = 0
						more.LayoutOrder = #show + 1
						more.ZIndex = 5
						CreateCorner(more, 4)
						CreatePadding(more, 0, 0, 6, 6)
						CreateStroke(more, theme().Accent, 1, 0.5)
						more.Parent = chipRow

						local lblM = Instance.new("TextLabel")
						lblM.Size = UDim2.new(0, 0, 1, 0)
						lblM.AutomaticSize = Enum.AutomaticSize.X
						lblM.BackgroundTransparency = 1
						lblM.Text = "+" .. hidden
						lblM.TextColor3 = t.TextSecondary
						lblM.FontFace = t.FontBold
						lblM.TextSize = 10
						lblM.ZIndex = 6
						lblM.Parent = more
					end
			...
			function Section:_addNestedDropdown(parent, dcfg)
				...
				local nObj = {
					Value = dcfg.Default or (not multi and opts[1]) or (multi and {}) or nil,
				}
				nObj._type = "Dropdown"

				local function fmtText()
					if multi then
						if type(nObj.Value) == "table" then
							local names = {}
							for k, v in pairs(nObj.Value) do
								if v == true then table.insert(names, tostring(k)) end
							end
						...
						return tostring(nObj.Value or "Select") .. " ▾"
					end
				end

				-- Compact header button (mirrors nested keybind layout)
				local dd = Instance.new("TextButton")
				dd.Size = UDim2.fromOffset(96, UI.SmallH)
				dd.Position = UDim2.new(1, -100, 0.5, -10)
				dd.BackgroundColor3 = t.Secondary
				dd.Text = fmtText()
				dd.TextColor3 = t.TextSecondary
				dd.FontFace = t.Font
				dd.TextSize = 10
				...
				CreateCorner(dd, 4)
				CreatePadding(dd, 0, 0, 6, 6)
				dd.Parent = parent

				-- Popup container (hidden until opened, anchored below the button)
				local popup = Instance.new("Frame")
				popup.Name = "NestedDDPopup"
				popup.AnchorPoint = Vector2.new(1, 0)
				popup.Size = UDim2.fromOffset(140, 0)
				popup.AutomaticSize = Enum.AutomaticSize.Y
				popup.BackgroundColor3 = t.Background
				popup.BorderSizePixel = 0
				popup.ZIndex = 50
				popup.Visible = false
				CreateCorner(popup, 6)
				CreateStroke(popup, t.Divider, 1, 0.4)
				CreatePadding(popup, 4, 4, 4, 4)
				local popLayout = CreateList(popup, 2)
				popup.Parent = dd
				registerThemed(popup, "BackgroundColor3", "Background")

				-- Position popup just below the button (manual because
				-- nested elements can't use AutomaticSize + AnchorPoint together
				-- cleanly without offset).
				...
				local function refreshVisual()
					dd.Text = fmtText()
					for _, rec in ipairs(optBtns) do
						local isOn
						if multi then
							isOn = type(nObj.Value) == "table" and nObj.Value[rec.opt] == true
						else
							isOn = nObj.Value == rec.opt
						end
						rec.btn.BackgroundColor3 = isOn and t.Accent or t.Background
						rec.btn.BackgroundTransparency = isOn and 0.2 or 1
						rec.btn.TextColor3 = isOn and t.TextPrimary or t.TextSecondary
					end
				end

				local function rebuild()
					for _, rec in ipairs(optBtns) do pcall(function() rec.btn:Destroy() end) end
					table.clear(optBtns)
					for i, opt in ipairs(opts) do
						local btn = Instance.new("TextButton")
						btn.Size = UDim2.new(1, 0, 0, UI.InputH - 4)
						...
						CreateCorner(btn, 4)
						btn.Parent = popup
						table.insert(optBtns, {btn = btn, opt = opt})

						btn.MouseButton1Click:Connect(function()
							if multi then
								if type(nObj.Value) ~= "table" then nObj.Value = {} end
								nObj.Value[opt] = not nObj.Value[opt] or nil
							else
								nObj.Value = opt
								popup.Visible = false
							end
							refreshVisual()
							if dcfg.Flag then updateFlag(dcfg.Flag, nObj.Value) end
							safeCallback(dcfg.Callback, nObj.Value)
						end)
					end
					refreshVisual()
				end
				...
				local closeConn = UserInputService.InputBegan:Connect(function(input)
					...
					local inPopup = m.X >= ap.X and m.X <= ap.X + as.X and m.Y >= ap.Y and m.Y <= ap.Y + as.Y
					local inHdr   = m.X >= dp.X and m.X <= dp.X + ds.X and m.Y >= dp.Y and m.Y <= dp.Y + ds.Y
					if not inPopup and not inHdr then popup.Visible = false end
				end)
				table.insert(_connections, closeConn)
				...
				function nObj:SetOptions(newOpts)
					opts = newOpts or {}
					rebuild()
				end
	...
	-- Live accent picker. Drops a Colorpicker into `tab` that re-tints
	-- every `tagAccent`-registered surface immediately. Preset swatches
	-- mirror the Tweaks panel from the HTML mockup.
	function Window:InsertAccentSection(tab, cfg)
		cfg = cfg or {}
		local sec = tab:CreateSection({Name = cfg.SectionName or "Акцент", Side = cfg.Side or "Right"})

		sec:Colorpicker({
			Name = cfg.Name or "Accent color",
			Flag = cfg.Flag,
			Default = SkrilyaLib:GetAccent(),
			Callback = function(c)
				SkrilyaLib:SetAccent(c)
			end,
		})

		-- Preset row — quick swap to named brand colors
		local presets = cfg.Presets or {
			{name = "Amethyst", color = Color3.fromRGB(182, 85, 247)},
			{name = "Cyan",     color = Color3.fromRGB(34, 211, 238)},
			{name = "Sunset",   color = Color3.fromRGB(245, 158, 11)},
			{name = "Forest",   color = Color3.fromRGB(16, 185, 129)},
		}
		for _, p in ipairs(presets) do
			sec:Button({
				Name = p.name,
				Callback = function()
					SkrilyaLib:SetAccent(p.color)
				end,
			})
		end
		return sec
	...
	function Window:CreateMinimizer(cfg)
		...
		local miniStroke = CreateStroke(mini, t.Accent, 1, 0.3)
		mini.Parent = gui
		tagAccent(mini, "BackgroundColor3")
		tagAccent(miniStroke, "Color")
	...
	function Window:SelectTab(indexOrTab)
		...
		if tab and tab._select then
			tab._select()
		elseif tab and tab._btn then
			-- Legacy fallback. MouseButton1Click:Fire doesn't work in
			-- most executors (RBXScriptSignal is read-only from Lua),
			-- but some patched signal impls accept it. Last resort.
			pcall(function() tab._btn.MouseButton1Click:Fire() end)
		end
	end
	...
	if config.LoadingEnabled then
		...
		tagAccent(progFill, "BackgroundColor3")
